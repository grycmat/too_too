...
Flutter Mastodon client ("Too Too") using Material 3 with a custom cyberpunk/neon-glow dark theme. Flutter SDK `^3.10.7`.
...
3. `OAuthCallbackHandler` listens for the `tootoo://oauth-callback?code=...` deep link via `app_links` and emits the code on a broadcast stream.
...
The custom URL scheme `tootoo://oauth-callback` must be registered in the native Android/iOS configs for the callback to work.

### Feature structure
Code is organized as `lib/features/<feature>/` with `<feature>_screen.dart`, optional `models/`, and `widgets/`. Shared cross-feature UI lives in `lib/core/widgets/` (e.g. `GlowWrapper`, `AppTextField`). Theme tokens live in `lib/core/theme/` (`AppColors`, `AppTheme.darkTheme`) — use these rather than hard-coding colors to keep the neon-glow aesthetic consistent.
